 ...
 * VolleyStat v0.0.9
 *
 * Fixes: Serve OUT button + counters
 * - Adds serveOut counter everywhere (storage, aggregation, derived metrics)
 * - Serve attempts = serve1 + serve2 + serve3 + ace + serveOut
 * - Pressure% = (serve3 + ace) / attempts
 */

const STORAGE_KEY = 'volleystat_v009';
...
// Serve weights are only for IN serves (serveOut is not scored).
...
function ensureCounters(team, match, set, playerId) {
  ...
  // Ensure new keys exist on old counter blobs
...
function warnMissing(id) {
  console.warn(`VolleyStat: missing #${id} in DOM`);
}
...
document.addEventListener('DOMContentLoaded', () => {
  const required = ['teamSelect', 'matchSelect', 'setSelect', 'viewSelect', 'statsBody'];
  const optional = [
    'teamsBtn',
    'rosterBtn',
    'undoBtn',
  ...
  const O = requireEls(optional).els;
  ...
  function recordEvent(action, playerId) {
    ...
    // Guard: allow serveOut even on older stored objects.
    if (counters[action] === undefined) {
      if (action === 'serveOut') {
        counters.serveOut = counters.serveOut ?? 0;
      } else {
