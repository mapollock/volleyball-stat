...
console.log('[VolleyStat] v0.1.11 loaded');

var STORAGE_KEY = 'volleystat_v011';
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function renderTable(){
    ...
    if (players.length > 1){
      ...
      nameCell.textContent = 'TEAM TOTALS';
      nameCell.className = 'left sticky-col';
      nameCell.style.fontWeight = '900';
      nameCell.style.background = '#f0f4ff';
      tfoot.appendChild(nameCell);

      tfoot.appendChild(td(String(totals.serveAtt)));
      tfoot.appendChild(td(String(totals.serveIn)));

      var pctTotCell = td(fmtPct(serveInPctTot));
      pctTotCell.style.fontWeight = '900';
      tfoot.appendChild(pctTotCell);

      if (uiMode !== 'coach'){
        tfoot.appendChild(td(String(totals.aceCount)));
        tfoot.appendChild(td(''));
      }

      tfoot.appendChild(td(String(totals.passAtt)));
      tfoot.appendChild(td(''));
      tfoot.appendChild(td(String(totals.hitAtt)));
      tfoot.appendChild(td(''));
      tfoot.appendChild(td(String(totals.hitsIn)));
      tfoot.appendChild(td(fmtPct(hitsInPctTot)));

      // Error totals — highlighted
      var errTotalCell = td(String(totals.totalErrors));
      errTotalCell.style.fontWeight = '900';
      errTotalCell.style.color = '#dc2626';
      tfoot.appendChild(errTotalCell);
      tfoot.appendChild(td(String(totals.errHitting)));
      tfoot.appendChild(td(String(totals.errServing)));
      tfoot.appendChild(td(String(totals.errPassing)));
      tfoot.appendChild(td(String(totals.errNet)));
      tfoot.appendChild(td(String(totals.errTwoHand)));
      tfoot.appendChild(td(String(totals.errRotation)));
  ...
  function buildEditGrid(){
    ...
    players.forEach(function(p){
      ...
      keys.forEach(function(key){
        ...
        var btn = document.createElement('button');
        btn.type = 'button';
        btn.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding:7px 10px;border:1px solid #e5e7eb;border-radius:8px;background:#fff;cursor:pointer;font-size:12px;font-weight:700;';
        btn.innerHTML = '<span>' + COUNTER_LABELS[key] + '</span><span style="background:#f3f4f6;border-radius:5px;padding:2px 7px;min-width:24px;text-align:center;">' + val + '</span>';
        if (val === 0) btn.style.opacity = '0.45';
        ...
        btn.addEventListener('click', function(){
          ...
          c[key] = c[key] - 1;
          // remove matching history entry (most recent for this player+action)
          ...
          btn.innerHTML = '<span>' + COUNTER_LABELS[key] + '</span><span style="background:#f3f4f6;border-radius:5px;padding:2px 7px;min-width:24px;text-align:center;">' + newVal + '</span>';
          btn.style.opacity = newVal === 0 ? '0.45' : '1';
        });
  ...
  import('./firebase-sync.js').then(function(fb) {
    // Expose save function globally so saveState() can call it
    window._firebaseSave = fb.firebaseSave;

    // Show device ID in the sync panel
    var devIdEl = byId('syncDeviceId');
    if (devIdEl) devIdEl.textContent = fb.getDeviceIdPublic();

    // Try loading from Firebase — if newer than local, use it
    fb.firebaseLoad().then(function(remote) {
      if (remote && remote.teams) {
        // Use remote if it has data and local doesn't, or always prefer remote
        var local = loadState();
      ...
      // Start real-time listener for other devices
    ...
    var copyDevBtn = byId('copyDeviceIdBtn');
    if (copyDevBtn) copyDevBtn.addEventListener('click', function(){
      navigator.clipboard && navigator.clipboard.writeText(fb.getDeviceIdPublic())
        .then(function(){ copyDevBtn.textContent = 'Copied!'; setTimeout(function(){ copyDevBtn.textContent = 'Copy ID'; }, 2000); });
    });
    var switchDevBtn = byId('switchDeviceBtn');
    var switchDevInput = byId('switchDeviceInput');
    if (switchDevBtn && switchDevInput) switchDevBtn.addEventListener('click', function(){
      if (!switchDevInput.value.trim()) return;
      if (confirm('Switch to device ID: ' + switchDevInput.value.trim() + '? This will reload the app and load that device data.')) {
        fb.switchDevice(switchDevInput.value.trim());
      }
    });
