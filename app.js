 ...
 *
 * Fixes included:
 * 1) Coach/Player toggle now works (updates labels + export headers)
 * 2) Serve OUT is a valid action and is included in serve attempts
 * 3) Legacy-safe: old saved teams/counters are merged with new keys on load
 *
 * Storage key remains the same as your current site to preserve saved teams.
 */
...
// UI Mode
const UI_MODE_KEY = 'volleystat_ui_mode';
...
const LABELS = {
  player: {
    ...
    passAvg: 'Pass Avg',
    hint:
      'Pressure% = (3 + ACE) ÷ Serve Attempts. Serving buttons reflect opponent pass: 1 (opp 3) → 3 (opp 1) → ACE (0). OUT counts as a serve attempt but not an opponent reception attempt.',
  },
  coach: {
    serveAtt: 'Opp SR Att',
    serveAvg: 'Opp SR Avg',
    ...
    passAvg: 'Our SR Avg',
    hint:
      'Opp OOS% = (Opp SR 1 + Opp SR 0) ÷ Opp SR Att. Serving buttons reflect opponent pass: 1 (opp 3) → 3 (opp 1) → ACE (0). OUT is a serve attempt but not an opponent reception attempt.',
  },
};
...
// Serve weights (OUT is not scored)
...
function ensureCounters(team, match, set, playerId) {
  ...
  // Legacy-safe add new keys
  team.data[match][set][playerId].serveOut ??= 0;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const setSelect = byId('setSelect');
  const viewSelect = byId('viewSelect');
  const statsBody = byId('statsBody');

  if (!teamSelect || !matchSelect || !setSelect || !viewSelect || !statsBody) {
    console.error('VolleyStat: required DOM elements missing; app init skipped.');
    return;
  }

  // Optional
  const teamsBtn = byId('teamsBtn');
  const rosterBtn = byId('rosterBtn');
  const undoBtn = byId('undoBtn');
  const exportName = byId('exportName');
  const exportBtn = byId('exportBtn');
  const resetBtn = byId('resetBtn');

  const onboarding = byId('onboarding');
  const onboardingTitle = byId('onboardingTitle');
  const onboardingSub = byId('onboardingSub');
  const onboardingTeamsBtn = byId('onboardingTeamsBtn');
  const onboardingRosterBtn = byId('onboardingRosterBtn');

  const exportConfirmBackdrop = byId('exportConfirmBackdrop');
  const exportConfirmClose = byId('exportConfirmClose');
  const exportConfirmCancel = byId('exportConfirmCancel');
  const exportConfirmOk = byId('exportConfirmOk');
  const exportConfirmFile = byId('exportConfirmFile');
  const exportConfirmTeam = byId('exportConfirmTeam');
  const exportConfirmView = byId('exportConfirmView');
  const exportConfirmScope = byId('exportConfirmScope');

  const pickerBackdrop = byId('pickerBackdrop');
  const pickerTitle = byId('pickerTitle');
  const playerGrid = byId('playerGrid');
  const pickerClose = byId('pickerClose');
  const pickerCancel = byId('pickerCancel');

  const rosterBackdrop = byId('rosterBackdrop');
  const rosterClose = byId('rosterClose');
  const rosterDone = byId('rosterDone');
  const rosterList = byId('rosterList');
  const playerForm = byId('playerForm');
  const playerIdEl = byId('playerId');
  const playerNameEl = byId('playerName');
  const playerNumberEl = byId('playerNumber');
  const playerPosEl = byId('playerPos');
  const newPlayerBtn = byId('newPlayerBtn');

  const teamsBackdrop = byId('teamsBackdrop');
  const teamsClose = byId('teamsClose');
  const teamsDone = byId('teamsDone');
  const teamsList = byId('teamsList');
  const teamForm = byId('teamForm');
  const teamIdEl = byId('teamId');
  const teamNameEl = byId('teamName');
  const newTeamBtn = byId('newTeamBtn');
  const exportTeamBtn = byId('exportTeamBtn');
  const importTeamBtn = byId('importTeamBtn');
  const importTeamInput = byId('importTeamInput');

  // Mode toggle + header ids
  const modePlayerBtn = byId('modePlayer');
  const modeCoachBtn = byId('modeCoach');
  const hintText = byId('hintText');
  const thServeAtt = byId('thServeAtt');
  const thServeAvg = byId('thServeAvg');
  const thAces = byId('thAces');
  const thAcePct = byId('thAcePct');
  const thPassAtt = byId('thPassAtt');
  const thPassAvg = byId('thPassAvg');
  const pressureLabel = byId('pressureLabel');

  let uiMode = loadUiMode();

  function applyModeToUI() {
    const L = LABELS[uiMode] || LABELS.player;
    if (thServeAtt) thServeAtt.textContent = L.serveAtt;
    if (thServeAvg) thServeAvg.textContent = L.serveAvg;
    if (pressureLabel) pressureLabel.textContent = L.pressure;
    if (thAces) thAces.textContent = L.aces;
    if (thAcePct) thAcePct.textContent = L.acePct;
  ...
  modePlayerBtn?.addEventListener('click', () => setUiMode('player'));
  modeCoachBtn?.addEventListener('click', () => setUiMode('coach'));
  ...
  function modeLabelForFile() {
    return uiMode === 'coach' ? 'CoachView' : 'PlayerView';
  }
  ...
  function openExportConfirmModal({ filename, teamName, viewLabel, scopeLabel }) {
    return new Promise((resolve) => {
      ...
      if (exportConfirmFile) exportConfirmFile.textContent = filename;
      if (exportConfirmTeam) exportConfirmTeam.textContent = teamName;
      if (exportConfirmView) exportConfirmView.textContent = viewLabel;
      if (exportConfirmScope) exportConfirmScope.textContent = scopeLabel;
  ...
  function getAggregateCounters(playerId) {
    ...
    function addFrom(matchName, setNum) {
      ...
      if (!c) return;
      const merged = { ...emptyCounters(), ...c };
      for (const k of Object.keys(agg)) agg[k] += merged[k] ?? 0;
    }
  ...
  function derived(playerId) {
    ...
    const serveOut = c.serveOut ?? 0;

    // Serve attempts include OUT
    const serveAtt = (c.serve1 ?? 0) + (c.serve2 ?? 0) + (c.serve3 ?? 0) + aceCount + serveOut;

    // Serve points ignore OUT
    const servePts =
  ...
  function recordEvent(action, playerId) {
    ...
    // Legacy-safe: merge defaults into this counter blob if action is missing
    if (counters[action] === undefined) {
      Object.assign(counters, { ...emptyCounters(), ...counters });
    }
  ...
  undoBtn?.addEventListener('click', () => {
    ...
    if (c && (c[last.action] ?? 0) > 0) c[last.action] -= 1;
    saveState();
    renderTable();
    updateOnboardingAndControls();
  });
  ...
  exportBtn?.addEventListener('click', async () => {
    ...
    const header =
      uiMode === 'coach'
        ? ['Team','Jersey','Player','Pos','Opp_SR_Att','Opp_SR_Avg','Opp_OOS%','Opp_SR_0','Opp_SR_0%','Our_SR_Att','Our_SR_Avg','HitAtt','HitAvg','Kill%']
        : ['Team','Jersey','Player','Pos','ServeAtt','ServeAvg','Pressure%','ACE','Ace%','PassAtt','PassAvg','HitAtt','HitAvg','Kill%'];
