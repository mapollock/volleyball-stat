...
console.log('[VolleyStat] v0.1.7.5 loaded');
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function exportCsvText(){
    ...
    if (uiMode === 'coach'){
      headers = [
        'Jersey', 'Player', 'Pos',
        'Opp SR Att', 'Opp SR Avg', 'Opp OOS%',
        'Our SR Att', 'Our SR Avg',
        'Hit Att', 'Hit Avg', 'Kills', 'Kill%'
      ];
    } else {
      headers = [
        'Jersey', 'Player', 'Pos',
        'Serve Att', 'Serve Avg', 'Serve In %',
        'Aces', 'Ace%',
        'Pass Att', 'Pass Avg',
        'Hit Att', 'Hit Avg', 'Kills', 'Kill%'
      ];
    ...
    for (var i=0;i<players.length;i++){
      ...
      if (uiMode === 'coach'){
        out.push([
          ...
          d.oppRecAtt, fmtNum(d.oppSrAvg,2), fmtPct(d.oppOosPct),
          d.passAtt, fmtNum(d.passAvg,2),
          d.hitAtt, fmtNum(d.hitAvg,3), d.kills, fmtPct(d.killPct)
        ].map(csv).join(','));
      } else {
        out.push([
          p.number || '', p.name || '', p.position || '',
          d.serveAttPlayer, fmtNum(d.serveAvgPlayer,2), fmtPct(d.serveInPct),
  ...
  function fireMobileAction(action){
    // Find desktop button with matching data-action and click it
    var desktopBtn = document.querySelector('.toolbar button[data-action="' + action + '"]');
    if (desktopBtn && !desktopBtn.disabled){
      desktopBtn.click();
      return;
    }
    // If no desktop button (e.g. mobile-only flow), dispatch directly on body
    // The existing app.js stat handler listens on document for data-action buttons
    var fakeBtn = document.createElement('button');
    fakeBtn.setAttribute('data-action', action);
  ...
  mobileStatBtns.forEach(function(btn){
    btn.addEventListener('click', function(){
      ...
      // Pre-select player if one is chosen in the dropdown
      if (mobileSelectedPlayerId && window._volleyStatSetPendingPlayer){
        window._volleyStatSetPendingPlayer(mobileSelectedPlayerId);
      }
