 ...
 * VolleyStat v0.0.8
 * Updates:
 *  1) Rename "Serve Pressure%" -> "Pressure%" (UI)
 *  2) Show ACE count as a column (ACE)
 *  3) Heatmap applies to Pressure% (not Serve Avg)
 */
...
function setToolbarStatsEnabled(enabled) {
  document.querySelectorAll(".toolbar button[data-action]").forEach(btn => btn.disabled = !enabled);
}
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Escape") { e.preventDefault(); closeExportConfirmModal(false); }
});
...
[matchSelect, setSelect, viewSelect].forEach(sel => sel?.addEventListener("change", () => { renderTable(); syncExportNameDefault(); }));
...
// Picker
function openPicker() { buildPlayerGrid(); pickerBackdrop.classList.remove("hidden"); }
function closePicker() { pickerBackdrop.classList.add("hidden"); pendingAction = null; }
...
function openRoster() { if (!activeTeam()) return; clearRosterForm(); renderRosterList(); rosterBackdrop.classList.remove("hidden"); }
function closeRoster() { rosterBackdrop.classList.add("hidden"); renderTable(); updateOnboardingAndControls(); }
...
function clearRosterForm() { playerIdEl.value = ""; playerNameEl.value = ""; playerNumberEl.value = ""; playerPosEl.value = ""; playerNameEl.focus(); }
...
function openTeams() { clearTeamForm(); renderTeamsList(); teamsBackdrop.classList.remove("hidden"); }
function closeTeams() { teamsBackdrop.classList.add("hidden"); initTeamSelect(); initMatchSelect(); renderTable(); exportName && (exportName.dataset.userEdited = ""); updateOnboardingAndControls(); }

temsCloseFix();
function temsCloseFix(){
...
function clearTeamForm() { teamIdEl.value = ""; teamNameEl.value = ""; teamNameEl.focus(); }
