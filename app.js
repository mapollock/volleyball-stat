 ...
 *
 * Fixes:
 * - Player/Coach mode toggle wired + persists
 * - Serve OUT is a valid action everywhere (counters + derived + legacy migration)
 * - Avoids optional catch binding (catch (e) {...}) to prevent syntax errors
 *
 * Storage: keeps volleystat_v008 to preserve existing local data.
 */

console.log('[VolleyStat] v0.1.2 loaded');
...
var SERVE_WEIGHTS = { serve1: 1, serve2: 2, serve3: 3, ace: 4 }; // OUT not scored
...
var LABELS = {
  player: {
    ...
    passAtt: 'Pass Att',
    passAvg: 'Pass Avg',
    hint: 'Pressure% = (3 + ACE) ÷ Serve Attempts. Serving buttons reflect opponent pass: 1 (opp 3) → 3 (opp 1) → ACE (0). OUT counts as a serve attempt but not an opponent reception attempt.'
  },
  coach: {
    serveAtt: 'Opp SR Att',
    serveAvg: 'Opp SR Avg',
    ...
    passAtt: 'Our SR Att',
    passAvg: 'Our SR Avg',
    hint: 'Opp OOS% = (Opp SR 1 + Opp SR 0) ÷ Opp SR Att. Serving buttons reflect opponent pass: 1 (opp 3) → 3 (opp 1) → ACE (0). OUT is a serve attempt but not an opponent reception attempt.'
  }
};
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function derived(playerId) {
    ...
    var aceCount = nz(c.ace, 0);
    var serveOut = nz(c.serveOut, 0);

    var serveAtt = nz(c.serve1,0) + nz(c.serve2,0) + nz(c.serve3,0) + aceCount + serveOut;
    var servePts = nz(c.serve1,0)*SERVE_WEIGHTS.serve1 + nz(c.serve2,0)*SERVE_WEIGHTS.serve2 + nz(c.serve3,0)*SERVE_WEIGHTS.serve3 + aceCount*SERVE_WEIGHTS.ace;
    var serveAvg = serveAtt ? (servePts / serveAtt) : 0;
    var acePct = safePct(aceCount, serveAtt);
    var pressurePct = safePct(nz(c.serve3,0) + aceCount, serveAtt);
    ...
    return { serveAtt: serveAtt, serveAvg: serveAvg, pressurePct: pressurePct, aceCount: aceCount, acePct: acePct, passAtt: passAtt, passAvg: passAvg, hitAtt: hitAtt, hitAvg: hitAvg, kills: kills, killPct: killPct };
  }
  ...
  function renderTable() {
    ...
    for (var i = 0; i < players.length; i++) {
      ...
      var pr = td(fmtPct(d.pressurePct));
      var heat = pressureHeatStyle(d.pressurePct);
