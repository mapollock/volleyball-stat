/**
 * VolleyStat v0.0.9
 * Based on the pre-change backup (v0.0.6) with serving updated to rating system.
 *
 * Serving (opponent pass quality):
 *  - Opponent passes 3  -> serve1 (easy)
 *  - Opponent passes 2  -> serve2
 *  - Opponent passes 1  -> serve3 (pressure)
 *  - Shank / no pass    -> ace
 *
 * Table updates:
 *  - Pressure% column (heatmap)
 *  - ACE count column
 *
 * NOTE: We keep STORAGE_KEY = volleystat_v006 for continuity and migrate old serveIn/serveOut.
 */
...
function emptyCounters() {
  return {
    ...
    // Legacy placeholders (for migration only)
    serveIn: 0, serveOut: 0,
...
function migrateServingCounters(c) {
  // If old serveIn/serveOut exist and new fields are zero, map serveIn -> serve2.
  // We cannot infer the opponent pass rating from old data; serve2 is a reasonable neutral default.
  if (!c) return;
  const hasOld = (c.serveIn || c.serveOut) && (c.serve1 === 0 && c.serve2 === 0 && c.serve3 === 0);
  if (hasOld) {
    c.serve2 += (c.serveIn || 0);
    c.ace += (c.ace || 0); // already in place
  }
}
...
// Updated serving actions: serve1/serve2/serve3/ace
